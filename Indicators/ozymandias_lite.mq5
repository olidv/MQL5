...
//---
   ...
   if(HMA_Handle==INVALID_HANDLE)
     {
      Print(" Não consegui entender o indicador do iMA");
      return(INIT_FAILED);
     }
//---
...
//+------------------------------------------------------------------+ 
int OnCalculate(const int rates_total,    // a quantidade de histórico em barras no tick atual
                const int prev_calculated,// a quantidade de histórico em barras no tick anterior
                const datetime &time[],
                const double &open[],
                const double &high[],
                const double &low[],
                ...
                const int &spread[])
  {
//--- verificando o número de barras quanto à suficiência para o cálculo
...
//--- cálculo do número limite inicial para o ciclo de recálculo da barra
   ...
   else limit=rates_total-prev_calculated;  // número inicial para calcular apenas novas barras
   to_copy=limit+1;
//--- copiar dados recentemente desenvolvidos para matrizes
   if(CopyBuffer(HMA_Handle,0,0,to_copy,HMA)<=0) return(RESET);
   if(CopyBuffer(LMA_Handle,0,0,to_copy,LMA)<=0) return(RESET);
//--- elementos de indexação em matrizes como em séries temporais
