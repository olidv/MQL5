...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CDayTrade::CDayTrade(void) : m_async_mode(false),
                       m_magic(0),
                       m_deviation(10),
...
//+------------------------------------------------------------------+
string CDayTrade::RequestActionDescription(void) const
  {
   string str;
//---
...
//+------------------------------------------------------------------+
string CDayTrade::RequestTypeDescription(void) const
  {
   string str;
//---
...
//+------------------------------------------------------------------+
string CDayTrade::RequestTypeFillingDescription(void) const
  {
   string str;
//---
...
//+------------------------------------------------------------------+
string CDayTrade::RequestTypeTimeDescription(void) const
  {
   string str;
//---
...
//+------------------------------------------------------------------+
string CDayTrade::ResultRetcodeDescription(void) const
  {
   string str;
//---
...
//| Open position                                                    |
//+------------------------------------------------------------------+
bool CDayTrade::PositionOpen(const string symbol,const ENUM_ORDER_TYPE order_type,const double volume,
                          const double price,const double sl,const double tp,const string comment)
  {
//--- check stopped
...
//+------------------------------------------------------------------+
bool CDayTrade::PositionModify(const string symbol,const double sl,const double tp)
  {
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//| Modify specified opened position                                 |
//+------------------------------------------------------------------+
bool CDayTrade::PositionModify(const ulong ticket,const double sl,const double tp)
  {
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
bool CDayTrade::PositionClose(const string symbol,const ulong deviation)
  {
...
//+------------------------------------------------------------------+
bool CDayTrade::PositionClosePartial(const string symbol,const double volume,const ulong deviation)
  {
   uint retcode=TRADE_RETCODE_REJECT;
//--- check stopped
   if(IsStopped(__FUNCTION__))
...
//+------------------------------------------------------------------+
bool CDayTrade::OrderOpen(const string symbol,const ENUM_ORDER_TYPE order_type,const double volume,const double limit_price,
...
//+------------------------------------------------------------------+
bool CDayTrade::OrderModify(const ulong ticket,const double price,const double sl,const double tp,
...
//+------------------------------------------------------------------+
void CDayTrade::PrintRequest(void) const
  {
...
//+------------------------------------------------------------------+
void CDayTrade::PrintResult(void) const
  {
   if(m_log_level<LOG_LEVEL_ALL)
      return;
//---
...
//| Buy operation                                                    |
//+------------------------------------------------------------------+
bool CDayTrade::Buy(const double volume,const string symbol=NULL,double price=0.0,const double sl=0.0,const double tp=0.0,const string comment="")
  {
//--- check volume
   if(volume<=0.0)
     {
...
//| Sell operation                                                   |
//+------------------------------------------------------------------+
bool CDayTrade::Sell(const double volume,const string symbol=NULL,double price=0.0,const double sl=0.0,const double tp=0.0,const string comment="")
  {
//--- check volume
   if(volume<=0.0)
     {
...
//+------------------------------------------------------------------+
bool CDayTrade::BuyLimit(const double volume,const double price,const string symbol=NULL,const double sl=0.0,const double tp=0.0,
                      const ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,const datetime expiration=0,const string comment="")
  {
   string symbol_name;
//--- check volume
...
//| Send BUY_STOP order                                              |
//+------------------------------------------------------------------+
bool CDayTrade::BuyStop(const double volume,const double price,const string symbol=NULL,const double sl=0.0,const double tp=0.0,
                     const ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,const datetime expiration=0,const string comment="")
  {
   string symbol_name;
//--- check volume
...
//+------------------------------------------------------------------+
bool CDayTrade::SellLimit(const double volume,const double price,const string symbol=NULL,const double sl=0.0,const double tp=0.0,
                       const ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,const datetime expiration=0,const string comment="")
  {
   string symbol_name;
//--- check volume
...
//+------------------------------------------------------------------+
bool CDayTrade::SellStop(const double volume,const double price,const string symbol=NULL,const double sl=0.0,const double tp=0.0,
                      const ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,const datetime expiration=0,const string comment="")
  {
   string symbol_name;
//--- check volume
...
//+------------------------------------------------------------------+
string CDayTrade::FormatPositionType(string &str,const uint type) const
  {
//--- see the type
   switch(type)
     {
...
//+------------------------------------------------------------------+
string CDayTrade::FormatOrderType(string &str,const uint type) const
  {
//--- see the type
   switch(type)
     {
...
//+------------------------------------------------------------------+
string CDayTrade::FormatOrderTypeFilling(string &str,const uint type) const
  {
//--- see the type
   switch(type)
     {
...
//+------------------------------------------------------------------+
string CDayTrade::FormatOrderTypeTime(string &str,const uint type) const
  {
//--- see the type
   switch(type)
     {
...
//+------------------------------------------------------------------+
string CDayTrade::FormatOrderPrice(string &str,const double price_order,const double price_trigger,const uint digits) const
  {
...
//+------------------------------------------------------------------+
string CDayTrade::FormatRequest(string &str,const MqlTradeRequest &request) const
  {
...
//+------------------------------------------------------------------+
string CDayTrade::FormatRequestResult(string &str,const MqlTradeRequest &request,const MqlTradeResult &result) const
  {
//--- set up
   string symbol_name=(request.symbol==NULL) ? _Symbol : request.symbol;
   int    digits=_Digits;
...
//|                                                                  |
//+------------------------------------------------------------------+
double CDayTrade::CheckVolume(const string symbol,double volume,double price,ENUM_ORDER_TYPE order_type)
  {
//--- check
   if(order_type!=ORDER_TYPE_BUY && order_type!=ORDER_TYPE_SELL)
      return(0.0);
...
//+------------------------------------------------------------------+
bool CDayTrade::OrderCheck(const MqlTradeRequest &request,MqlTradeCheckResult &check_result)
  {
//--- action and return the result
...
//+------------------------------------------------------------------+
bool CDayTrade::SetTypeFillingBySymbol(const string symbol)
  {
...
//+------------------------------------------------------------------+
bool CDayTrade::FillingCheck(const string symbol)
  {
...
//+------------------------------------------------------------------+
bool CDayTrade::ExpirationCheck(const string symbol)
  {
//--- check symbol
   string symbol_name=(symbol==NULL) ? _Symbol : symbol;
//--- get flags
...
//| Checks order                                                     |
//+------------------------------------------------------------------+
bool CDayTrade::OrderTypeCheck(const string symbol)
  {
   bool res=false;
//--- check symbol
   string symbol_name=(symbol==NULL) ? _Symbol : symbol;
...
//| Send order                                                       |
//+------------------------------------------------------------------+
bool CDayTrade::OrderSend(const MqlTradeRequest &request,MqlTradeResult &result)
  {
   bool   res;
   string action="";
   string fmt   ="";
...
//| Position select depending on netting or hedging                  |
//+------------------------------------------------------------------+
bool CDayTrade::SelectPosition(const string symbol)
  {
   bool res=false;
//---
   if(IsHedging())
